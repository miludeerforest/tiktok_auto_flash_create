...
- **自动验证码可选开启**：默认关闭，只有手动勾选后才会尝试自动处理拼图型滑块验证码
...
- `auto_solve_captcha`：是否启用自动验证码处理，默认 `false`
...
1. 填写 4 条参考活动名称
2. 如需尝试自动处理拼图滑块验证码，手动勾选 **启用自动验证码处理（默认关闭）**
3. 点击 **扫描实例**
4. 从下拉框选择店铺实例
5. 点击 **检测当前实例**
6. 点击 **保存配置**
...
- 默认情况下不会自动尝试验证码求解
- 如果你手动开启了 **启用自动验证码处理（默认关闭）**，程序会先尝试自动处理，再决定是否进入人工流程
- GUI 会显示更明显的右上角提醒通知
- 请切到浏览器手动完成验证/关闭弹窗
- 回到程序点击 **手动继续**
